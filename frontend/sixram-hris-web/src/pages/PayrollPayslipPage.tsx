...
export function PayrollPayslipPage() {
  ...
  async function loadPayslip(id: string) {
    ...
    try {
      const response = await sixramApi.getPayslip(id)
      setPayslip(response)
      setError(null)
    } catch (caughtError) {
      ...
      <section className="shell-card fade-up p-6 sm:p-7 print:shadow-none">
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <SummaryCard label="Net Pay" value={formatCurrency(payslip.netPay, payslip.currency)} tone="success" />
          </div>
        </div>
      </section>
