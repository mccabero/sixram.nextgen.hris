...
{
    Task<StoredLeaveAttachmentFile> SaveAsync(Guid employeeId, Guid leaveRequestId, IFormFile file, CancellationToken cancellationToken = default);

    Task<StreamedLeaveAttachmentFile> OpenReadAsync(string storagePath, string downloadFileName, string mimeType, CancellationToken cancellationToken = default);

    Task DeleteAsync(string? storagePath, CancellationToken cancellationToken = default);
}

public sealed record StoredLeaveAttachmentFile(
    string OriginalFileName,
    string StoragePath,
    long FileSize,
    string MimeType);

public sealed record StreamedLeaveAttachmentFile(
    Stream Content,
    string ContentType,
    string DownloadFileName);

public class LeaveAttachmentStorageService : ILeaveAttachmentStorageService
{
    ...
    private readonly ILogger<LeaveAttachmentStorageService> _logger;

    public LeaveAttachmentStorageService(
        IOptions<LeaveOptions> options,
        IWebHostEnvironment environment,
        ILogger<LeaveAttachmentStorageService> logger)
    {
    ...
    public async Task<StoredLeaveAttachmentFile> SaveAsync(Guid employeeId, Guid leaveRequestId, IFormFile file, CancellationToken cancellationToken = default)
    {
        ValidateFile(file);
        ...
        var storageFileName = $"{leaveRequestId:N}-{Guid.NewGuid():N}{extension}";
        ...
        var absoluteDirectory = Path.GetDirectoryName(absolutePath)
            ?? throw new InvalidOperationException("The leave attachment directory could not be resolved.");
        ...
        _logger.LogInformation("Stored leave attachment for request {LeaveRequestId} at {StoragePath}.", leaveRequestId, storagePath);

        return new StoredLeaveAttachmentFile(
    ...
    public Task<StreamedLeaveAttachmentFile> OpenReadAsync(string storagePath, string downloadFileName, string mimeType, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new NotFoundException("The requested leave attachment could not be found in storage.");
        }
        ...
        return Task.FromResult(new StreamedLeaveAttachmentFile(
            Content: stream,
    ...
    public Task DeleteAsync(string? storagePath, CancellationToken cancellationToken = default)
    {
        ...
        if (File.Exists(absolutePath))
        {
            ...
            _logger.LogInformation("Deleted leave attachment file {StoragePath}.", storagePath);
        }
    ...
    private void ValidateFile(IFormFile file)
    {
        ...
        if (file.Length > _options.MaxAttachmentSizeBytes)
        {
            throw BuildValidationException(
                $"The uploaded file exceeds the {_options.MaxAttachmentSizeMb} MB limit.",
                nameof(file));
        }
    ...
    private string NormalizeMimeType(string extension, string? contentType)
    {
        ...
        {
            throw BuildValidationException("The uploaded file type does not match the file extension.", nameof(contentType));
        }
    ...
    private string ResolveAbsolutePath(string storagePath)
    {
        ...
        {
            throw new BadRequestException("The leave attachment storage path is invalid.");
        }
